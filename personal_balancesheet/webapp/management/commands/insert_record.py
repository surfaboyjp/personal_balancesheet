...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        for x in journal_list:
            ...
            user = User.objects.get(id=journal.user.id)

            asset = list(Asset.objects.filter(user=user).all().values())
            ...
            liability = list(Liability.objects.filter(user=user).all().values())
            ...
            income = list(Income.objects.filter(user=user).all().values())
            ...
            cost = list(Cost.objects.filter(user=user).all().values())
